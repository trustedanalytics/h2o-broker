...
public class KerberosConfigTest {
    ...
    public void isKerberosEnabled_allPropertiesAreEmptyOptionals_returnsFalse() throws Exception {
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_KDC)).thenReturn(Optional.empty());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_REALM)).thenReturn(Optional.empty());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.USER)).thenReturn(Optional.empty());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.PASSWORD)).thenReturn(Optional.empty());
        boolean kerberosEnabled = kerberosConfig.isKerberosEnabled();
        assertThat(kerberosEnabled, is(false));
    }
    ...
    public void isKerberosEnabled_allPropertiesReadThrowException_returnsFalse() throws Exception {
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_KDC)).thenThrow(new IOException());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_REALM)).thenThrow(new IOException());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.USER)).thenThrow(new IOException());
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.PASSWORD)).thenThrow(new IOException());
        boolean kerberosEnabled = kerberosConfig.isKerberosEnabled();
        assertThat(kerberosEnabled, is(false));
    }

    private void arrangeTestEnvironment(String kdc, String realm, String user, String password) throws IOException {
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_KDC)).thenReturn(Optional.of(kdc));
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.KRB_REALM)).thenReturn(Optional.of(realm));
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.USER)).thenReturn(Optional.of(user));
        when(configurationHelper.getPropertyFromEnv(PropertyLocator.PASSWORD)).thenReturn(Optional.of(password));
    }
}
